...
async function handleRequest(request) {
  ...
  if (path === '/' || path === '/index.html') {
    return fetch('https://your-github-raw-content-url/index.html')
  } else if (path === '/styles.css') {
    return fetch('https://your-github-raw-content-url/styles.css')
  } else if (path === '/script.js') {
    return fetch('https://your-github-raw-content-url/script.js')
  } else if (path === '/api/links') {
    if (request.method === 'GET') {
      const links = await LINKS_KV.get('all_links', 'json') || getDefaultLinks();
      return new Response(JSON.stringify(links), {
        headers: { 'Content-Type': 'application/json' },
      })
    } else if (request.method === 'POST') {
      const data = await request.json()
      let links = await LINKS_KV.get('all_links', 'json') || getDefaultLinks();
      ...
      await LINKS_KV.put('all_links', JSON.stringify(links))
